	...
	const Dropdown = Spicetify.React.memo(({ name, title, desc, options, defaultVal, condition = true, tippy, callback }) => {
		...
		Spicetify.React.useEffect(() => {
			if (isFirstRender.current) {
				isFirstRender.current = false;
				if (!startup) {
					const parent = document.querySelector('[aria-label="Masafy Settings"]');
			...
			Spicetify.React.createElement(Section, { name: "Cover Art" }, [
				{
					...
					tippy: Spicetify.React.createElement(
						...
						Spicetify.React.createElement("li", null, "Masafy default: (60px, 60px, 8px, 20px)"),
					...
					items: [
						...
						},
						{
							...
							tippy: Spicetify.React.createElement(
								...
								Spicetify.React.createElement("li", null, "Masafy default: 20px"),
